...
<task type="auto">
  ...
  <action>
    Add async def call_llm_vision(messages: list[dict], image_base64: str | None, session_id: str | None = None) -> str.
    ...
    Call litellm.acompletion with model=CODE_GEN_MODEL, messages=messages, temperature=0.3, max_tokens=8000. Do NOT pass response_format or response_format={"type": "json_object"}.
    ...
    **Logging** (per AGENTS.md LLM call rules — use session_id as context instead of journey_id):
    - Before call: `log("INFO", "llm vision call started", session_id=session_id, provider=CODE_GEN_MODEL, prompt_type="design_to_code", has_image=bool(image_base64))`
    - On success: `log("INFO", "llm vision call succeeded", session_id=session_id, provider=CODE_GEN_MODEL, duration_ms=..., tokens_used=..., output_length=len(content))`
    - On failure: `log("ERROR", "llm vision call failed", session_id=session_id, provider=CODE_GEN_MODEL, error=str(e), error_code=...)`
    - On empty content: `log("WARN", "llm vision returned empty content", session_id=session_id, provider=CODE_GEN_MODEL)`
  </action>
  <verify>call_llm_vision with text-only messages returns content. With image_base64, litellm accepts multimodal content. No response_format crash. Logs visible.</verify>
  <done>call_llm_vision works for multimodal input with structured logging.</done>
</task>
