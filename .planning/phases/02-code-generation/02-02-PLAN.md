...
<task type="auto">
  ...
  <action>
    ...
    **Logging** (import `from app.config import log`):
    - On entry: `log("INFO", "design context transform started", node_count=len(nodes), frame_name=...)`
    - On completion: `log("INFO", "design context transform completed", tree_depth=..., node_count_output=..., icon_count=..., image_count=..., duration_ms=...)`
    - On malformed/empty input: `log("WARN", "design context transform empty input")`
  </action>
  <verify>Call transform_design_context with sample dict from figma.py import response. Output has frame, tree, tree nodes have layout/style/content where applicable. No crash on empty or malformed input. Log output visible on console.</verify>
  <done>transform_design_context produces compact structure for LLM with structured logging.</done>
</task>
